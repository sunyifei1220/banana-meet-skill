...
首次需要调用 MCP（`init`、`create` 或 `report`）而 MCP 尚未可用时，Skill 优先引导使用者在 Codex App 的 **Settings → MCP Servers** 添加 `banana-meet` 远程服务并填写 Bearer Token；只有该界面不可用或不支持静态 Token 时，才提供终端备用命令。成功接入后不会在后续操作中重复引导。
